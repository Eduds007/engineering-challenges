...
- `output/<siren>/`: same pipeline run against the other 19 companies (exploratory, not
  the graded deliverable, the graded output is `results.json` at the root for
  480489707 only).
...
to an OCR line's bounding box, which is what makes an event "grounded" rather than merely
plausible.
...
| OpenRouter (`poolside/laguna-s-2.1:free`, primary provider) | ~50 to 80 (estimated) | ~40,000 (estimated) | n/a | n/a | ~15,000 (estimated) | $0 (free tier) |
